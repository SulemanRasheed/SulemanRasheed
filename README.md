...
This project benchmarked multiple EEG-based brain–computer interface (BCI) pipelines for classifying hand-grasp intentions in stroke patients. Using Python (NumPy, SciPy, MNE, and scikit-learn), I implemented and compared five popular algorithms — including classical signal-processing methods (Wavelet Transform, PSD), motor imagery-specific techniques (CSP, FBCSP), and the EEGNet deep neural network.

The study revealed that FBCSP remains the most effective for within-subject decoding (≈ 84.8% accuracy), while EEGNet excels in cross-subject generalisation (≈ 77.0% accuracy). These findings provide an open benchmark for selecting suitable EEG pipelines in assistive robotics and rehabilitation BCIs.
